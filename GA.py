    ...
    def __init__(self, width, height):
        ...
        for i in range(0, self.num):
            ...
            self.triangles.append(triangle(
                (point1x, point1y), (point2x, point2y), (point3x, point3y), (r, g, b)))

    def draw(self, width, height):
        self.im = Image.new('RGB', (width, height), "#FFFFFF")
        # 绘制100个半透明三角形重叠的图片，将三角形一个一个的累加
        for triangle in self.triangles:
            # 将一个三角形画出
            temp_im = Image.new('RGB', (width, height), "#000000")
            imDraw = ImageDraw.Draw(temp_im)
            imDraw.polygon([triangle.point1, triangle.point2,
                            triangle.point3], triangle.rgb)
            mask = Image.new("L", (width, height), 255)
            maskDraw = ImageDraw.Draw(mask)
            maskDraw.polygon([triangle.point1, triangle.point2,
                              triangle.point3], fill=128)
            # 将当前绘制出的三角形图像和原图形融合 注意只将当前图像上三角形区域和原图像对应区域融合
            # 否则当前三角形图像的黑色背景会不断稀释原图像的三角形颜色
            self.im = composite(self.im, temp_im, mask)
            # self.im = blend(self.im, temp_im, 0.5)
...
class GA():
    ...
    def __init__(self):
        ...
        self.target = background.resize((32, 32))
    ...
    def mutation(self):
        ...
        for i in range(chromosome_num):
            ...
            if rand < self.mut_rate:
                ...
                g = random.randint(0, 255)
                b = random.randint(0, 255)
                self.chromosomes[i].triangles[mut_pos] = triangle(
                    (point1x, point1y), (point2x, point2y), (point3x, point3y), (r, g, b))
